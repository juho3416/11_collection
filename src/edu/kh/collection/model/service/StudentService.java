...
public class StudentService {
	...
	//List<Student> : Student로 저장되는 타입이 제한된 리스트 생성
...
//		studentList.add(new Object()); // 3번 인덱스
		...
		// 반환형이 Object == 모든 객체를 반환할 수 있다는 말
		
		
		if(studentList.get(0) instanceof Student) { //get(0)은 배열의 0번 인덱스 값을 가져온다는 의미.
			System.out.println( ((Student) studentList.get(0)).getName() ); //Student로 다운캐스팅 후 getName
		}
		
		// 제네릭스 (Generics)
		// 보통 제네릭
		// -> 컬렉션에 저장되는 객체 타입을 한가지로 제한
		System.out.println( studentList.get(0).getName()); // 제네릭 쓰면 Student만 들어오게 되므로 다운캐스팅 안해도 됨.
	...
	public String addStudent() throws InputMismatchException{
		...
		System.out.print("점수 : ");
		int score = sc.nextInt(); //문자열로 받으면 InputMismatchException
...
//		}
	...
	public String updateStudent() throws InputMismatchException {
		...
		// -> List의 i번째 요소를 전달받은 e로 변경 한다는 메소드로
	...
	public String removeStudent() throws InputMismatchException {
		...
		// 4) 입력받은 숫자가 studentList 범위 내 인덱스 번호인가?
			...
			//		String 대문자 -> 대문자 0번 인덱스 
			//		문자 y->Y / Y->Y
			
			// String.tpUpperCase() : 문자열을 대문자로 변경
...
// 내가 푼것	
//	public String searchName1() throws InputMismatchException {	
//		
//		// 검색할 이름 입력 : 
//		
...
//		
//		for (int i = 0; i<studentList.size(); i++) {
//			if ( inputName.equals( studentList.get(i).getName() ) ) {
...
//	}
	...
	public void searchName2() {
		...
		for (Student std : studentList) {
			...
			// boolean String.contains(문자열) : String에 문자열이 포함되어 있으면 true 아니면 false 반환
